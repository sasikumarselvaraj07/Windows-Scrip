...
New-Item -path \\192.168.129.238\Scripts\$computer -type Directory -ErrorAction Ignore
...
ConvertTo-HTML -Head $Style -PostContent "$ReportHead $OSHead $OS $DiskHead $Disk $cpuHead $cpuUsage $memoryHead $totmemory $TotalMemoryinMb $tm $usedmemory $Mem $AppLogHead $AppEvent $SysLogHead $SysEvent $ServHead $Service $InstalledAppsHead $InstalledApps $HotFixHead $HotFix" -Title "System Health Check Report"  |  Out-File "\\192.168.129.238\Scripts\$computer\Health Report $CurrentDate.html"