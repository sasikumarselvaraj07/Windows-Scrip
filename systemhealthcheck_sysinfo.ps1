...
$procHead = ConvertTo-HTML -AS Table -Fragment -PreContent '<H2>Processor</H2>'|Out-String  
$DiskHead = ConvertTo-HTML -AS Table -Fragment -PreContent '<H2>Disk Information</H2>'|Out-String 
$cpuHead = ConvertTo-HTML -AS Table -Fragment -PreContent '<H2>CPU Utlization</H2>'|Out-String
$memoryHead = ConvertTo-HTML -AS Table -Fragment -PreContent '<H2>Memory Utlization</H2>'|Out-String
$totmemory = ConvertTo-HTML -AS Table -Fragment -PreContent '<H4>Total Memory is</H4>' |Out-String
$usedmemory = ConvertTo-HTML -AS Table -Fragment -PreContent '<H4>Used Memory</H4>'|Out-String
...
$test = ConvertTo-HTML -AS Table -Fragment -PreContent '<p>Total Memory is $mem </p>' |Out-String
...
New-Item -path C:\$computer -type Directory -ErrorAction Ignore
...
$CpuPercentFree = 
@{
  Expression = {[int]($CpuUsage*100/$_.Size)}
  Name = 'Free (%)'
}
...
# Check CPU Usage
$cpuUsage = Get-Counter '\Processor(_Total)\% Processor Time'  | Select-Object -ExpandProperty CounterSamples | Select-Object -ExpandProperty CookedValue  | ConvertTo-HTML -Fragment


#Check Memory Usage
#$memoryUsage = Get-Counter '\Memory\Available MBytes' | Select-Object -ExpandProperty CounterSamples | Select-Object -ExpandProperty CookedValue | ConvertTo-HTML -Fragment
#$Mem = "$memoryUsage MB"
    $computer = gc env:computername
...
ConvertTo-HTML -Head $Style -PostContent "$ReportHead $OSHead $OS $procHead $proc $DiskHead $Disk $cpuHead $ComputerCpu $memoryHead $totmemory $TotalMemoryinMb $tm $usedmemory $Mem $AppLogHead $AppEvent $SysLogHead $SysEvent $ServHead $Service $InstalledAppsHead $InstalledApps $HotFixHead $HotFix" -Title "System Health Check Report"  |  Out-File "C:\$computer\Health Report $CurrentDate.html"



#------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

#to collect systeminfo into txt format
# Run systeminfo command and store the output
$systemInfoOutput = systeminfo

# Specify the path where you want to save the output file
$outputFilePath = "C:\$computer\SystemInfo.txt"

# Save the systeminfo output to a text file
$systemInfoOutput | Out-File -FilePath $outputFilePath -Encoding UTF8

Write-Host "System information saved to: $outputFilePath"